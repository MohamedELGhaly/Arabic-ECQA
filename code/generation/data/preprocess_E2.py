...
def make_data_file(filename_in, filename_out):
	...
	for i in range(0,len(q_texts)):
		...
		bos_token = '<BOS>'
		eos_token = '<EOS>'
		text = bos_token + ' سؤال: '+q_text+' '
		if correct:
			text += 'الجواب هو ' + option + ' لأن '
		else:
			text += 'الجواب ليس ' + option + ' لأن '
...
def make_test_file(filename_in, filename_out):
	...
	for i in range(0,len(q_texts)):
		...
		text = bos_token + ' سؤال: ' + q_text + ' '
		if correct:
			text += 'الجواب هو ' + option + ' لأن <BOP> '
		else:
			text += 'الجواب ليس ' + option + ' لأن <BOP> '
		for j in range(0,len(q_properties)):
			property = q_properties[j]
			if j==0:
				target = property + ' <EOP> '
...
def make_noisy_test_file(filename_in, filename_out):
	...
	for i in range(0,len(q_texts)):
		...
		text = bos_token + ' سؤال: ' + q_text + ' '
		if correct:
			text_correct = text + 'الجواب هو ' + option + ' لأن <BOP> '
			text_noisy = text + 'الجواب ليس ' + option + ' لأن <BOP> '
		else:
			text_correct = text + 'الجواب ليس ' + option + ' لأن <BOP> '
			text_noisy = text + 'الجواب هو ' + option + ' لأن <BOP> '
		for j in range(0,len(q_properties)):
				property = q_properties[j]
				if j==0:
					target = property + ' <EOP> '
